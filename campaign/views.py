    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            campaign = Campaign.objects.create(campaign_name = data.get("campaign_name"),
                                            ...
                                            templateresource =data.get("templateresource"),
...
class CSVUploadView(APIView):
    ...
    def post(self , request , *args, **kwargs):
        ...
        if serializer.is_valid():           
            data = serializer.validated_data
            targetusercsv = TargetUserCSV.objects.create( file_name = data.get("file_name"))
            ...
            with open(targetuserfile.file_name.path,'r') as f:
                ...
                for i, row in enumerate(reader):
                    if i == 0:
                        ...
                        row = "".join(row)
                        email = row
                        targetusergroup = TargetUserGroup.objects.get(id =request.data.get('id'))
                        targetuser = TargetUser.objects.create(targetusergroup = targetusergroup)
...
class SendTemplateMailView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        mail_list = target_mail_list.replace('"','')
        targetuser_mail_list = mail_list.split(",")
        ...
        new_added_mail_list = [x for x in targetuser_mail_list if x not in email_list]
        ...
        for email_id in targetuser_mail_list:
            ...
            print(targetuser)
            subject, from_email, to = subject_message , config('EMAIL_HOST_USER'),  [email_id]
...
class TargetUserCredentials(APIView):
    ...
    def post(self, request , *args, **kwargs):
        ...
        targetuser = TargetUser.objects.get(target_user_uuid = request.data.get("target_user_uuid"))
        targetuser.email_credential = request.data.get("email_credential")
        targetuser.password_credential = request.data.get("password_credential")
        targetuser.save()
...
class ValidateTemplate(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            campaign = Campaign.objects.get(id =campaign_id)
            if campaign:
                target_user_uuid = TargetUser.objects.get(target_user_uuid = target_user_uuid)
                if target_user_uuid:
                    template_resource = TemplateResource.objects.get(template_name = template_resource)
                    if template_resource:
                        serializer = TemplateResourceUpdateSerializer(template_resource)
